...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = $conn->prepare("SELECT unique_id, password, company_rep_id FROM delivery WHERE email = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $delivery['password'])) {
            ...
            $_SESSION['unique_id'] = $delivery['unique_id'];
            $_SESSION['company_rep_id'] = $delivery['company_rep_id']; // Store the company_rep_id
...
<br>
    ...
    <div class="input-container">
        <form action="login.php" method="POST">
            <h3> Login for Delivery Person </h3>
